...
import Card, { ContentProduct } from "../Card";
...
export default function ListProducts({
  ...
  const getListProducts = ({ page, rows, sortBy, orderBy }: ParamsApi) => {
    ...
    url += `?page=${page}&rows=${rows}&sortBy=${sortBy}&orderBy=${orderBy}`;
  ...
  useEffect(() => {
    let params: ParamsApi = { page: 1, rows: 8, sortBy: "id", orderBy: "DESC" };
    getListProducts(params);
  }, []);

  return (
    <section>
      <div className="w-2/3 flex flex-col md:grid grid-cols-4 justify-items-center items-center gap-5 h-full">
